    ...
    if (this.getTimeDurationError(start, end) === '') {
      ...
      if (
        ...
        if (this.timeRangeErrorString === 'Empty End date time supplied.') {
          ...
          this.timeRangeErrorString = `Time range set to last 24 hrs. Start and End date time must not be more than ${(
            this.allowedDurationInDays * 24
          ).toString()} hrs apart, Start date must be within the past 30 days and end date must be 15 minutes less than the current time.`;
          this._endTime = moment.utc().subtract(16, 'minutes');
