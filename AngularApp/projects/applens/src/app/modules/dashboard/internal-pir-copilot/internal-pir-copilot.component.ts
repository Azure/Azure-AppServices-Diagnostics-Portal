...
import { TelemetryService, ChatUIContextService, TelemetryEventNames, APIProtocol, ChatModel } from 'diagnostic-data';
...
@Component({
  selector: 'internal-pir-copilot',
  templateUrl: './internal-pir-copilot.component.html',
  styleUrls: ['./internal-pir-copilot.component.scss']
})
export class InternalPirCopilotComponent implements OnInit {

  userPhotoSource: string = '';
  userNameInitial: string = '';
  chatHeader: string = `<h1 class='chatui-header-text'><b>Internal PIR Co-Pilot</b>!</h1>`;

  // Variables to be passed down to the OpenAI Chat component
  chatComponentIdentifier: string = "internalpircopilot";
  ...
  chatQuerySamplesFileURIPath = "assets/chatConfigs/internalpircopilot.json"; 
...
//custom copy method/callback 
  copyChatGPTClicked = (textToCopy: string) => {
    ...
    PortalUtils.logEvent("internalpircopilot-messagecopied", textToCopy, this._telemetryService); 
  }
